...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "LAPORAN KUESIONER MAHASISWA"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$sql = mysqli_query($koneksi,"SELECT * FROM mahasiswa,prodi where prodi_id=mahasiswa_prodi");
$no = 1; // Untuk penomoran tabel, di awal set dengan 1
$numrow = 4; // Set baris pertama untuk isi tabel adalah baris ke 4

while($data = mysqli_fetch_array($sql)){
  $excel->setActiveSheetIndex(0)->setCellValue('A'.$numrow, $no);
  $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['mahasiswa_nama']);
  $excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $data['mahasiswa_npm']);
  $excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $data['prodi']);
  $excel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$numrow, $data['mahasiswa_email'], PHPExcel_Cell_DataType::TYPE_STRING);
  ...
  $id_mahasiswa = $data['mahasiswa_id'];
  $huruf = "F";
  $polling = mysqli_query($koneksi,"SELECT * FROM polling,mahasiswa,pertanyaan,jawaban WHERE mahasiswa_id=polling_mahasiswa AND mahasiswa_id='$id_mahasiswa' AND polling_pertanyaan=pertanyaan_id AND polling_jawaban=jawaban_id");
